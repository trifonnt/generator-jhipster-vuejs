...
const getters = {
	currentEntity(state) {
		return state.entities[state.currentEntity];
	},
	isChecked: (state)=>(id, entity)=>{
		return state.entities[entity].checkedDeleted.find(pid=>pid==id) || false;
	},
	allChecked: (state)=>(entity)=> {
		return state.entities[entity].checkedDeleted.length;
		//return state.entity.length == state.entity.checkedDeleted.length
	}
}

	const actions = {
		async getData({state, commit}, {entity, page, sort, search, rowsPerPage, labels, options, values}) {
	        try {
	          let sort = sort || '';
