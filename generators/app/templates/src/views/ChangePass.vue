        ...
        <v-layout justify-center>
          <v-flex xs12 sm8 md4>
            <v-card class="elevation-12">
              ...
              <v-card-text>
                <v-form>
                  <v-text-field ref='password' v-validate="{required: true, min: 4, max: 100}" prepend-icon="lock" name="cpassword" label="Current Password" type="password" v-model="cpassword"></v-text-field>
                    ...
                    <v-alert type='error' :value="reset==false">Incorrect password</v-alert>      
                </v-form>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn :disabled="errors.any() || !formValid" color="primary" @click='change'>Send</v-btn>
              </v-card-actions>
            </v-card>
          </v-flex>
        </v-layout>
